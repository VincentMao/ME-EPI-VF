...
class Mapping(object):
    ...
    def T1_estimate(self, num_set):
        ...
        set_len = len(num_set);
        degree_dif = float(self.nflip_num / (self.origflipset[0]-self.origflipset[-1]));
        num_ntr = int(degree_dif)/2;
        # Option 1
...
#                idx_ix = idx_ix - 1;
            # Option 2
            idx_ix = int((idx_i+1)*degree_dif);
            idx_iy = int((idx_i+1)*degree_dif - num_ntr);
            flip_idx = int((idx_i+1)*degree_dif - num_ntr/2);
            xdata[:,:,:,idx_k] = np.mean(self.imgdata[:,:,:,idx_ix:idx_iy:-1], axis=3) / math.tan(self.flipset[flip_idx]);
            ydata[:,:,:,idx_k] = np.mean(self.imgdata[:,:,:,idx_ix:idx_iy:-1], axis=3) / math.sin(self.flipset[flip_idx]);
            idx_k = idx_k + 1;
...
def main(argv):
    ...
    parser.add_option('-i', '--input',
                      dest='infile',
                      default='/echo_001_num600_dataset_brain.nii.gz',
    ...
    parser.add_option('-f', '--filename',
                      dest='filename',
                      default='T1_estimate_num600_Andy_orig_v2.nii.gz',
    ...
    # Flip angle set
    flipset = np.linspace(80,35,nflip_num);
    # Repetition Time
    TR = 1000;
    mapping = Mapping(filename, flipset, nflip_num, TR, outdir, options.filename);
    num_set = np.array([0,4,9,14,19,24,29,34,39,44]);
